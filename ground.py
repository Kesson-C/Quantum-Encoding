...
class groupedFermionicOperator:
    ...
    """
    def __init__(self, ferOp, num_electron, labeling_method=None, mode='uhf'):
        """
    ...
    def to_paulis(self):
        ...
        def add_helper(queue, w, arg1, arg2): # the func called in child processes
            ret = w*arg1*arg2
            queue.put(ret)
        ...
        for k, w in self.grouped_op.items():
            if np.ndim(k) == 1: ## one-e-term
                ...
                k1, k2 = k
                if k1[0]==k1[1] or k2[0]==k2[1] or k1[0]!=k2[0] or k1[1]!=k2[1]:
                    # qubitOp += (w * mapping[k1] * mapping[k2])
                    p = Process(target=add_helper, args=(q, w, mapping[k1], mapping[k2]))
                    processes.append(p)
                    p.start()
            else:
